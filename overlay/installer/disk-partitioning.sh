# Forked from omarchy-iso (github.com/omacom-io/omarchy-iso,
# configs/airootfs/usr/share/omarchy-iso/disk-partitioning.sh, MIT license,
# (c) Anton Hvornum). This file is upstream's disk machinery verbatim below
# the marker — the never-predict-a-partition-number rule, creation
# bookkeeping, and rollback. It is arch-neutral by design; the Pi-specific
# choices (MBR label, 512M FAT boot + ext4 root, PARTUUID boot chain) live in
# omarchy-cm5-install-to-disk.sh, which sources this.
